...
def clicked_lives():
    lives.configure(text="Number of Lives: "+str(var3.get()),\
...
def checked(event):
    ...
    while run:
        try:
            ...
            # Using if block to validate that the input given from the entrybox 
            # is correct by the answer variable determined from above functions.
            if int(entry.get()) == answer:
                indication = "CORRECT"
                
                # Adds 1 to correct and attempted variables so that there is 
                # a runnning total.
                correct += 1
                atemp_made += 1
                ...
                #Adds 1 to incorrect and attempted variables so that 
                # there is a runnning total.
                incorrect += 1
                atemp_made += 1
...
def reset():
    ...
    lives_choice = Checkbutton(tab1, state=DISABLED) # unchecking the checkbox

#Help function, which if pressed, will give all of the instructions to the game
def helped():
     messagebox.showinfo('Help',\
                'Ahoy!, welcome to the Pirates of the Caribbean Game! \n\n'\
